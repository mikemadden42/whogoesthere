...
- [ ] **Generalize `resolve_benign_alias` beyond `dbus-org.*` to cover any
      `systemctl enable` symlink under `/etc/systemd/{system,user}`.** The
      Ubuntu 24.04 capture shows the dbus-org pattern is a special case of
      a much broader one: roughly 30 of the 38 systemd UNTRACKED findings
      are enable-symlinks (`sshd.service`, `samba.service`, `iscsi.service`,
      `syslog.service`, `smartd.service`, `ntpd.service`, `vmtoolsd.service`,
      `display-manager.service`, `dbus-fi.w1.wpa_supplicant1.service`, etc.)
      pointing into `/usr/lib/systemd/system/`. `display-manager.service` is
      UNTRACKED on Fedora for the same reason. Fix: drop the
      `is_fedora_dbus_alias` discriminator and instead reattribute through
      *any* `/etc/systemd/{system,user}/*.{service,timer,path,socket}`
      symlink whose canonicalized target is itself package-owned. Security
      property still holds — a malicious symlink to `/tmp/evil.service`
      doesn't resolve to an owned target and stays UNTRACKED. Tag with
      `benign_pattern: systemd-enable-symlink` and `alias_target: <path>`
      so the reattribution is auditable. Expected effect: Ubuntu UNTRACKED
      ~84 → ~54; Fedora ~19 → ~15 (`intel_lpmd.service` is third-party
      install and correctly stays).
